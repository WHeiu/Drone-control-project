...
//	I2C_ReadBytes(I2C1,MAG_ADDRESS,MAG_XOUT_L + 2 * i,2,(uint8_t*)(Mag + i),LSB);//Get The X Y Z Mag
	  SPI_MAG_ReadBytes(MAG_XOUT_L + 2*i ,2,(uint8_t*)(Mag + i), LSB);
		...
		sprintf( TEST7, "%f", Mag[0]); 
		sprintf( TEST8, "%f", Mag[1]); 
		sprintf( TEST9, "%f", Mag[2]); 
	  printff(USART3, "\r\nACC:\r\n",strlen("\r\nACC:\r\n"));
...
//	GYO[i] -= Gyo_Offset[i];//Calibrate
	...
	Mahony_update(GYO[0]* Factor,GYO[1]* Factor,GYO[2] * Factor,Acc[0] * Gravity,Acc[1]* Gravity,Acc[2]* Gravity,Mag[0] ,Mag[1] ,Mag[2] );
	Mahony_CAngles();
//	Mahony_CAngles();
	LL_mDelay(2);
//	ReadPressure();
