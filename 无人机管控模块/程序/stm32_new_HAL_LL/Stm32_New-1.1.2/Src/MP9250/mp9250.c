...
extern	char Pitch_[100];
extern  char Roll_[100];
extern	char Yaw_[100];
...
uint8_t MPU9250_Write_LEN(uint8_t addr,uint8_t reg,uint8_t len,uint8_t *buf)
{
	uint8_t i;
	MPU_9250_ENABLE;   //	MPU9250_CS=0;  //片选MPU9250
//	status=SPI1_ReadWriteByte(reg); //发送reg地址
 ...
 void i2c_Mag_write(uint8_t reg,uint8_t value)
{
	...
	char test[10] = {0};
	uint8_t res = 0;
	printff(USART3, "----------配置I2C_MAG_Write!!!---------\r\n\r\n",strlen("----------配置I2C_MAG_Write!!!---------\r\n\r\n"));	 
	MPU9250_Write_Reg(I2C_SLV0_ADDR ,MPU9250_AK8963_ADDR);//设置磁力计地址,mode: write  地址是0x0C 
	res = MPU9250_Read_Reg(I2C_SLV0_ADDR);
	  if(res == MPU9250_AK8963_ADDR)
		{
		  	printff(USART3, "设置I2C_SLV0_ADDR磁力计地址成功!!!\r\n\r\n",strlen("设置I2C_SLV0_ADDR磁力计地址成功!!!\r\n\r\n"));	 
	    	printff(USART3, "I2C_SLV0_ADDR 的值为：",strlen("I2C_SLV0_ADDR 的值为：n"));	 				
				sprintf( test,"%0x",res);
		...
		{
		  	printff(USART3, "设置I2C_SLV0_ADDR磁力计地址失败!!!\r\n\r\n",strlen("设置I2C_SLV0_ADDR磁力计地址失败!!!\r\n\r\n"));	 	
				printff(USART3, "I2C_SLV0_ADDR 的值为：",strlen("I2C_SLV0_ADDR 的值为：n"));	 				
				sprintf( test,"%0x",res);
				printff(USART3, test,strlen(test));	
				printff(USART3, "\r\n\r\n",strlen("\r\n\r\n"));	
	...
	MPU9250_Write_Reg(I2C_SLV0_REG ,reg);//set reg addr
		res = MPU9250_Read_Reg(I2C_SLV0_REG);
	  if(res == reg)
		{
		  	printff(USART3, "设置I2C_SLV0_REG磁力计寄存器成功!!!\r\n\r\n",strlen("设置I2C_SLV0_REG磁力计寄存器成功!!!\r\n\r\n"));	 
	    	printff(USART3, "I2C_SLV0_REG 的值为：",strlen("I2C_SLV0_REG 的值为：n"));	 				
				sprintf( test,"%0x",res);
				printff(USART3, test,strlen(test));	
				printff(USART3, "\r\n\r\n",strlen("\r\n\r\n"));	
		}
		...
		{
		  	printff(USART3, "设置I2C_SLV0_REG磁力计寄存器失败!!!\r\n\r\n",strlen("设置I2C_SLV0_REG磁力计寄存器失败!!!\r\n\r\n"));	 	
				printff(USART3, "I2C_SLV0_REG 的值为：",strlen("I2C_SLV0_REG 的值为：n"));	 				
				sprintf( test,"%0x",res);
				printff(USART3, test,strlen(test));	
				printff(USART3, "\r\n\r\n",strlen("\r\n\r\n"));	
	...
	MPU9250_Write_Reg(I2C_SLV0_DO ,value);//send value	
		res = MPU9250_Read_Reg(I2C_SLV0_DO);
	  if(res == value)
		{
		  	printff(USART3, "设置I2C_SLV0_DO磁力计寄存器的值成功!!!\r\n\r\n",strlen("设置I2C_SLV0_DO磁力计寄存器的值成功!!!\r\n\r\n"));	 
	    	printff(USART3, "I2C_SLV0_DO 的值为：",strlen("I2C_SLV0_DO 的值为：n"));	 				
				sprintf( test,"%0x",res);
				printff(USART3, test,strlen(test));	
				printff(USART3, "\r\n\r\n",strlen("\r\n\r\n"));	
		}
		...
		{
		  	printff(USART3, "设置I2C_SLV0_DO磁力计寄存器的值失败!!!\r\n\r\n",strlen("设置I2C_SLV0_DO磁力计寄存器的值失败!!!\r\n\r\n"));	 	
				printff(USART3, "I2C_SLV0_DO 的值为：",strlen("I2C_SLV0_DO 的值为：n"));	 				
				sprintf( test,"%0x",res);
				printff(USART3, test,strlen(test));	
				printff(USART3, "\r\n\r\n",strlen("\r\n\r\n"));	
	...
	LL_mDelay (10);
	MPU9250_Write_Reg(I2C_SLV0_CTRL ,0X81);//send value	
		res = MPU9250_Read_Reg(I2C_SLV0_CTRL);
	  if(res == 0X81)
		{
		  	printff(USART3, "设置I2C_SLV0_CTRL磁力计控制寄存器成功!!!\r\n\r\n",strlen("设置I2C_SLV0_CTRL磁力计控制寄存器成功!!!\r\n\r\n"));	 
	    	printff(USART3, "I2C_SLV0_CTRL 的值为：",strlen("I2C_SLV0_CTRL 的值为：n"));	 				
				sprintf( test,"%0x",res);
				printff(USART3, test,strlen(test));	
				printff(USART3, "\r\n\r\n",strlen("\r\n\r\n"));	
		}
		...
		{
		  	printff(USART3, "设置I2C_SLV0_CTRL磁力计控制寄存器失败!!!\r\n\r\n",strlen("设置I2C_SLV0_CTRL磁力计控制寄存器失败!!!\r\n\r\n"));	 	
				printff(USART3, "I2C_SLV0_CTRL 的值为：",strlen("I2C_SLV0_CTRL 的值为：n"));	 				
				sprintf( test,"%0x",res);
				printff(USART3, test,strlen(test));	
				printff(USART3, "\r\n\r\n",strlen("\r\n\r\n"));	
		...
		printff(USART3, "-----------------------------------------\r\n\r\n",strlen("-----------------------------------------\r\n\r\n"));	 
	LL_mDelay (100);//此处因为MPU内部I2C读取速度较慢，延时等待内部写完毕
}
/***************************************************************/
// MPU内部i2c 读取
//I2C_SLVx_ADDR:  MPU9250_AK8963_ADDR
...
//	uint16_t j=5000;
	...
	LL_mDelay (10);
	LL_mDelay (100);//此处因为MPU内部I2C读取速度较慢，必须延时等待内部读取完毕
	return MPU9250_Read_Reg(EXT_SENS_DATA_00);
}
 uint8_t i2c_Mag_read1(uint8_t addr,uint8_t reg)
{
//	uint16_t j=5000;
	MPU9250_Write_Reg(I2C_SLV0_ADDR ,addr|0x80); //设置磁力计地址，mode：read
	LL_mDelay (10);
	MPU9250_Write_Reg(I2C_SLV0_REG ,reg);// set reg addr
	LL_mDelay (10);
	MPU9250_Write_Reg(I2C_SLV0_DO ,0xff);//read
...
//	MPU9250_Write_Reg(CONFIG, 0x07);      //低通滤波频率，典型值：0x07(3600Hz)此寄存器内决定Internal_Sample_Rate==8K
	MPU9250_Write_Reg(CONFIG, 0x01);      //低通滤波频率，典型值：0x07(3600Hz)此寄存器内决定Internal_Sample_Rate==8K
/**********************Init SLV0 i2c**********************************/	
//Use SPI-bus read slave0
	MPU9250_Write_Reg(INT_PIN_CFG ,0x12);// INT Pin / Bypass Enable Configuration  
	MPU9250_Write_Reg(I2C_MST_CTRL,0x0d);//I2C MAster mode and Speed 400 kHz
	MPU9250_Write_Reg(USER_CTRL ,0x30); // I2C_MST_EN 
...
//	MPU9250_Write_Reg(GYRO_CONFIG, 0x18); //陀螺仪自检及测量范围，典型值：0x18(不自检，2000deg/s)
	MPU9250_Write_Reg(GYRO_CONFIG, 0x10); //陀螺仪自检及测量范围，典型值：0x18(不自检，2000deg/s)
//	MPU9250_Write_Reg(ACCEL_CONFIG, 0x18);//加速计自检、测量范围及高通滤波频率，典型值：0x18(不自检，16G)
	MPU9250_Write_Reg(ACCEL_CONFIG, 0x08);
//	MPU9250_Write_Reg(ACCEL_CONFIG_2, 0x08);//加速计高通滤波频率 典型值 ：0x08  （1.13kHz）	
	MPU9250_Write_Reg(ACCEL_CONFIG_2, 0x04);//加速计高通滤波频率 典型值 ：0x08  （1.13kHz）	

/**********************Init MAG **********************************/
	i2c_Mag_write(0X0B,0X01);    //复位磁力计
	if( i2c_Mag_read(0X0B)== 0X01)
		 printff(USART3, "复位成功!!!\r\n",strlen("复位成功!!!\r\n"));	
	LL_mDelay(100);
	i2c_Mag_write(0x0a,0x1f);
	LL_mDelay(100);
	res	= i2c_Mag_read(0x00); //读取磁力计ID  0x00 为磁力计ID寄存器	
	if(res == 0x48)  //等待读取磁力计ID
	{
	 printff(USART3, "成功读取磁力计ID!!!\r\n",strlen("成功读取磁力计ID!!!\r\n"));	   
	 ...
	 if( i2c_Mag_read(AK8963_CNTL2_REG)== AK8963_CNTL2_SRST)
		 printff(USART3, "复位成功!!!\r\n",strlen("复位成功!!!\r\n"));	
	 i2c_Mag_write(AK8963_CNTL1_REG,0x0F); // 16位输出， 单次测量模式
...
void MPU9250_Get_Data(void)
{
   ...
   //  while(mpu_mpl_get_data(&pitch,&roll,&yaw)!=0 );   //这里是while()空循环，等待读取数据
	...
	sprintf( Pitch_, "%f", pitch ); 
	sprintf( Roll_, "%f", roll ); 
	sprintf( Yaw_, "%f", yaw);
	
	printff(USART3, "Pitch:\r\n",strlen("Pitch:\r\n"));
	printff(USART3, Pitch_,strlen(Pitch_));
	printff(USART3, "\r\n\r\nRoll:\r\n",strlen("\r\n\r\nRoll:\r\n"));
	printff(USART3, Roll_,strlen(Roll_));
	printff(USART3, "\r\n\r\nYaw:\r\n",strlen("\r\n\r\nYaw:\r\n"));
	printff(USART3, Yaw_,strlen(Yaw_));
	printff(USART3, "\r\n", strlen("\r\n"));
...
//	printff(USART3,"**************MPU9250**************\r\n",strlen("**************MPU9250**************\r\n"));
//  Sensor_Read();
//	sprintf( Pitch_, "%f", getPitch() ); 
//	sprintf( Roll_, "%f", getRoll() ); 
//	sprintf( Yaw_, "%f", getYaw());
//	
//	printff(USART3, "Pitch:\r\n",strlen("Pitch:\r\n"));
//	printff(USART3, Pitch_,strlen(Pitch_));
//	printff(USART3, "\r\n\r\nRoll:\r\n",strlen("\r\n\r\nRoll:\r\n"));
