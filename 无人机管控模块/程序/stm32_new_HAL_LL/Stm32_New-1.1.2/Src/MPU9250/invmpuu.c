 ...
 * min(int a, int b)
 */
#if defined EMPL_TARGET_STM32F4     //编译器宏定义
//#include "main.h"
#include "logg.h"
...
//    int result = 0x07;
//    short data;
//	  unsigned char tiaoshi=0;
//    char test[20]={0};
// mpu_set_bypass(1); //  设置为直通模式
// //   mpu_set_bypass(0);   //应该为非直通模式
//	
//		printff(USART3, "11111111111!!!\r\n",strlen("11111111111!!!\r\n"));	   
//		
...
//	 i2c_Mag_write(AKM_REG_CNTL,tmp[0]);
//	  tiaoshi = i2c_Mag_read(AKM_REG_WHOAMI);
//   sprintf( test, "%u", tiaoshi); 
//	 printff(USART3, test,strlen(test));	
////    if (i2c_write(st.chip_cfg.compass_addr, AKM_REG_CNTL, 1, tmp))
////        return 0x07;
//	  
//  	printff(USART3, "222222222222222222!!!\r\n",strlen("222222222222222222!!!\r\n"));	   		
//		
//    tmp[0] = AKM_BIT_SELF_TEST;
//	  i2c_Mag_write(AKM_REG_ASTC,tmp[0]);
...
//	 //    goto AKM_restore;
//	  
//		printff(USART3, "33333333333!!!\r\n",strlen("33333333333!!!\r\n"));	   		

//    do {
//        delay_ms(10);
//  //      if (i2c_read(st.chip_cfg.compass_addr, AKM_REG_ST1, 1, tmp))
//			       //     goto AKM_restore;
//			tmp[0] = i2c_Mag_read(AKM_REG_ST1);
...
//		printff(USART3, "444444444444444444!!!\r\n",strlen("444444444444444444!!!\r\n"));	   		
//  //  if (i2c_read(st.chip_cfg.compass_addr, AKM_REG_HXL, 6, tmp))
//		tmp[0] = i2c_Mag_read(0x03);
//		tmp[1] = i2c_Mag_read(0x04);
//		tmp[2] = i2c_Mag_read(0x05);
//		tmp[3] = i2c_Mag_read(0x06);
//		tmp[4] = i2c_Mag_read(0x07);
//		tmp[5] = i2c_Mag_read(0x08);
//      //  goto AKM_restore;
...
int mpu_run_6500_self_test(long *gyro, long *accel, unsigned char debug)
{
   ...
   // 	log_i("Starting MPU6500 HWST!\r\n");
    printff(USART3, "Starting MPU6500 HWST!\r\n",strlen("Starting MPU6500 HWST!\r\n")); 
    if (st.chip_cfg.dmp_on) {
        mpu_set_dmp_state(0);
        dmp_was_on = 1;
    } else
    ...
    printff(USART3, "Retrieving Biases\r\n",strlen("Retrieving Biases\r\n")); 
    ...
    if (ii == tries) {
       ...
       printff(USART3, "Retrieving Biases Error - possible I2C error\r\n",strlen("Retrieving Biases Error - possible I2C error\r\n")); 
        result = 0;
        goto restore;
    }
  ...
  printff(USART3, "Retrieving ST Biases\r\n",strlen("Retrieving ST Biases\r\n")); 
    for (ii = 0; ii < tries; ii++)
  ...
  printff(USART3, "Retrieving ST Biases Error - possible I2C error\r\n",strlen("Retrieving ST Biases Error - possible I2C error\r\n")); 
    ...
    if (!gyro_result)
		{
        result |= 0x01;
			  printff(USART3, "gyro_result\r\n",strlen("gyro_result\r\n")); 
		}
    if (!accel_result)
		{
        result |= 0x02;
		  	printff(USART3, "accel_result\r\n",strlen("accel_result\r\n")); 
		}

#ifdef AK89xx_SECONDARY
    compass_result = compass_self_test();
		
		sprintf( test1, "%u", compass_result); 
    ...
    if (!compass_result)
		{
        result |= 0x04;
			  printff(USART3, "compass_result\r\n",strlen("compass_result\r\n")); 
		}
#else
    result |= 0x04;
#endif
 ...
 int setup_compass(void)
{
    unsigned char data[4], akm_addr;
		...
		uint8_t state;
//    mpu_set_bypass(1);	
	  printff(USART3, "****************setup_compass****************\r\n",strlen("****************setup_compass****************\r\n"));	   
//		MPU9250_Write_Reg(MPU_USER_CTRL_REG,0X20);  //I2C主模式开启  Isolate I2C for compass
//	  MPU9250_Write_Reg(I2C_MST_CTRL,0x0d);//I2C MAster mode and Speed 400 kHz
...
//			data[0] = i2c_Mag_read1( akm_addr,AKM_REG_WHOAMI);
...
//			printff(USART3, "Compass not found.!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\r\n",strlen("Compass not found.!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\r\n"));	   
//        return -1;
//    }
...
////        return -1;
//		i2c_Mag_write(AKM_REG_CNTL,data[0]);
//    delay_ms(1);
...
//        return -1;
//	dataa = MPU9250_SPI_Read(MPU9250_INT_PIN_CFG);    
//  LL_mDelay(1);
//	MPU9250_SPI_Write(MPU9250_INT_PIN_CFG, (0xfd & dataa));   // DISable Bypass模式
//	LL_mDelay(1);
...
//	else
	...
	printff(USART3, "设置磁力计FUSE ROM模式成功\r\n",strlen("设置磁力计FUSE ROM模式成功\r\n"));
	MPU9250_AK8963_SPI_Read_LEN(MPU9250_AK8963_I2C_ADDR, MPU9250_AK8963_ASAX, 3, data);
	LL_mDelay(10);
//		data[0] = i2c_Mag_read(AKM_REG_ASAX);
//		data[1] = i2c_Mag_read(0x11);
//		data[2] = i2c_Mag_read(0x12);
		
		sprintf( TEST10, "%u", data[0]); 
  	sprintf( TEST11, "%u", data[1]); 
	  sprintf( TEST12, "%u", data[2]); 
	  printff(USART3, "setup_compass response:\r\n",strlen("setup_compass response:\r\n"));
...
//    mpu_set_bypass(0);
    ...
    if (i2c_write(st.hw->addr, st.reg->i2c_delay_ctrl, 1, data))
        return -1;
 printff(USART3, "****************setup_compass  END****************:\r\n",strlen("****************setup_compass  END****************\r\n"));
#ifdef MPU9150
  ...
  char test[10]={0};
   ...
   sprintf( test, "%u", res); 
	 printff(USART3, test,strlen(test));	 	
		...
		if(res)
		{
			printff(USART3, "run_self_test error!!!\r\n",strlen("run_self_test error!!!\r\n"));	 	
			return 10;
		}			
...
uint8_t mpu_mpl_get_data(float *pitch,float *roll,float *yaw)
{
	...
	{
		printff(USART3, "DMP 读取 FIFO 失败!!!\r\n",strlen("DMP 读取 FIFO 失败!!!\r\n"));	 
		return 1;	
	} 
